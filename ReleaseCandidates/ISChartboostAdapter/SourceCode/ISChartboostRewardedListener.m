//
//  ISChartboostRewardedListener.m
//  ISChartboostAdapter
//
...
@implementation ISChartboostRewardedListener
...
- (void) didCacheAd:(CHBCacheEvent *)event
    ...
    if (error) {
        [_delegate didFailToCacheRewardedVideo:_locationId
                                     withError:error];
    } else {
        [_delegate didCacheRewardedVideo:_locationId];
    }
}
...
- (void) didShowAd:(CHBShowEvent *)event
    ...
    if (error) {
        [_delegate didFailToShowRewardedVideo:_locationId
                                    withError:error];
    } else {
        [_delegate didDisplayRewardedVideo:_locationId];
    }
}
...
- (void) didClickAd:(CHBClickEvent *)event
    ...
    [_delegate didClickRewardedVideo:_locationId];
}
...
- (void) didEarnReward:(CHBRewardEvent *)event {
    [_delegate didCompleteRewardedVideo:_locationId];
}
...
- (void) didDismissAd:(CHBDismissEvent *)event {
    [_delegate didDismissRewardedVideo:_locationId];
}
